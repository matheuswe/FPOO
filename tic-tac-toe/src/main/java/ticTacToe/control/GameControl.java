...
public class GameControl {
	...
	private void verifyWinnerAtLines() {
		...
		for(int lin=0; lin<3; lin++)
			if(haveWinner(getMark(lin, 0), getMark(lin, 1), getMark(lin, 2)))setGameOver(getMark(lin,0));
	}
	
	private void verifyWinnerAtColumns() {
		
		for(int col=0; col<3; col++)
			if(haveWinner(getMark(0, col), getMark(1, col), getMark(2, col)))setGameOver(getMark(0,col));
	}
